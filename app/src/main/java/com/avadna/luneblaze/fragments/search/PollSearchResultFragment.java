...
public class PollSearchResultFragment extends Fragment
    ...
    public PollSearchResultFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (position == 4) {
            offset = 0;
            hitSearchPostsApi(user_id, query, offset, "poll");
        }
        return baseLayout;
    }
    ...
    private void hitSearchPostsApi(final String user_id, final String query, final int localOffset, String type) {
        ll_no_connection_wrapper.setVisibility(View.GONE);
            ...
            call.enqueue(new Callback<PojoSearchResultResponse>() {
                ...
                public void onResponse(Call<PojoSearchResultResponse> call, Response<PojoSearchResultResponse> response) {
                    ...
                    if (response != null && response.body() != null) {
                        ...
                        if (response.body().data.poll != null) {
                            if (response.body().data.poll.isEmpty()) {
                                if (offset == 0) {
                                    newsFeedAdapter.setShowNoMoreResults(true, getString(R.string.no_results_found));
                                } else {
                                    newsFeedAdapter.setShowNoMoreResults(true, getString(R.string.no_more_results_found));
                                }
                            }
                            newsFeedList.addAll(response.body().data.poll);
                        }
                        newsFeedAdapter.notifyDataSetChanged();
                    ...
                    if (searchActivity != null) {
                        ...
                        if (!latestQuery.equals(query)) {
                            offset = 0;
                            hitSearchPostsApi(user_id, PollSearchResultFragment.this.query, offset,
                                    "poll");
                        }
                    }
                    newsFeedAdapter.setShowProgressBar(false);
