...
public class ProfileInfoInitiatedSessionListAdapter extends RecyclerView.Adapter<ProfileInfoInitiatedSessionListAdapter.MyViewHolder> {
    //  pivate List<String> hierarchyList;
    int listSize = 0;
    Activity activity;
    List<PojoSessionInitiated> profileInfoSessionList;
    ...
    public ProfileInfoInitiatedSessionListAdapter(Activity activity, List<PojoSessionInitiated> profileInfoSessionList) {
    ...
    @Override
    public ProfileInfoInitiatedSessionListAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.profile_info_session_list_item, parent, false);

        return new ProfileInfoInitiatedSessionListAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final ProfileInfoInitiatedSessionListAdapter.MyViewHolder holder, int position) {
        ...
        if (position == profileInfoSessionList.size()) {
            ...
            final PojoSessionInitiated currentItem = profileInfoSessionList.get(position);

            holder.tv_session_name.setText(Html.fromHtml(currentItem.title));
            if (currentItem.totalAttends.equals("0")) {
                holder.tv_num_attendees.setVisibility(View.GONE);
            } else {
                holder.tv_num_attendees.setVisibility(View.VISIBLE);
