...
public class SessionPhotoUploadAdapter extends RecyclerView.Adapter<SessionPhotoUploadAdapter.MyViewHolder> {
    //  pivate List<String> hierarchyList;
    int type;
    List<PojoVenuePhoto> photoList;
    Activity activity;
    ...
    SessionPhotoUploadAdapterCallback sessionPhotoUploadAdapterCallback;

    public SessionPhotoUploadAdapter(Activity activity, List<PojoVenuePhoto> photoList, String session_id) {
        try {
            this.sessionPhotoUploadAdapterCallback = ((SessionPhotoUploadAdapterCallback) activity);
        } catch (ClassCastException e) {
            throw new ClassCastException("Activity must implement  SessionPhotoUploadAdapterCallback.");
        }
    ...
    @Override
    public SessionPhotoUploadAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.session_photo_upload_item, parent, false);

        return new SessionPhotoUploadAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final SessionPhotoUploadAdapter.MyViewHolder holder, final int position) {
        //   holder.tv_hierarchy_item.setText(hierarchyList.get(position));
        PojoVenuePhoto currentItem = photoList.get(position);
      ...
      /*  Picasso.get()
        ...
        holder.iv_photo.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String dataStr = new Gson().toJson(photoList, type);
                ...
                imgViewerIntent.putExtra(AppKeys.SESSION_ID, session_id);
                imgViewerIntent.putExtra(AppKeys.SESSION_NAME, AppKeys.SESSION);
                activity.startActivity(imgViewerIntent);
                // openImageGalleryDialog(position);
            }
        });
        // item is fetched from api
        if (currentItem.uploadStatus == null && currentItem.photoname != null) {
        ...
        holder.ib_delete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                sessionPhotoUploadAdapterCallback.onSessionPhotoUploadAdapterItemClick(position, AppKeys.ACTION_DELETE_PHOTO);
            }
        });
        ...
        holder.tv_retry.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (holder.tv_retry.getText().equals(activity.getString(R.string.retry))) {
                    sessionPhotoUploadAdapterCallback.onSessionPhotoUploadAdapterItemClick(position, AppKeys.ACTION_RETRY_UPLOAD);
                } else if (holder.tv_retry.getText().equals(activity.getString(R.string.cancel))) {
                    sessionPhotoUploadAdapterCallback.onSessionPhotoUploadAdapterItemClick(position, AppKeys.ACTION_CANCEL_UPLOAD);
                }
            }
        });
    ...
    public static interface SessionPhotoUploadAdapterCallback {
        void onSessionPhotoUploadAdapterItemClick(int position, String type);
    }
