...
public class UpcomingSessionAdapter extends RecyclerView.Adapter<UpcomingSessionAdapter.MyViewHolder> {
    //  pivate List<String> hierarchyList;
    int listSize = 0;
    Activity activity;
    List<PojoUpcomingSession> initiationSessions = new ArrayList<>();
    ...
    public UpcomingSessionAdapter(Activity activity, List<PojoUpcomingSession> initiationSessions, boolean fixed_venue) {
        this.activity = activity;
        this.initiationSessions = initiationSessions;
    ...
    @Override
    public UpcomingSessionAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.upcoming_session_list_item, parent, false);

        return new UpcomingSessionAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final UpcomingSessionAdapter.MyViewHolder holder, int position) {
        //   holder.tv_hierarchy_item.setText(hierarchyList.get(position));

        if (position < initiationSessions.size()) {
            ...
            final PojoUpcomingSession currentItem = initiationSessions.get(position);
       ...
       /* if(activity!=null) Glide.with(activity.getApplicationContext())
            ...
            holder.tv_sharer_place_holder.setVisibility(View.GONE);
            holder.tv_sharer_name.setVisibility(View.GONE);
            ...
            if (!currentItem.presentors.isEmpty() && currentItem.sharer.get(0).userId.equals(user_id)) {
            ...
            holder.tv_attend_button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (commonFunctions.isGuestUser()) {
                        ...
                        if (currentItem.iAttend) {
                            Intent verifiedIntent = new Intent(activity, VerifiedSessionActivity.class);
                            verifiedIntent.putExtra("id", currentItem.sessionsId);
                            verifiedIntent.putExtra("type", AppKeys.UNATTEND);
                            activity.startActivity(verifiedIntent);
                            ...
                            Intent verifiedIntent = new Intent(activity, VerifiedSessionActivity.class);
                            verifiedIntent.putExtra("id", currentItem.sessionsId);
                            verifiedIntent.putExtra("type", AppKeys.ATTEND);
                            activity.startActivity(verifiedIntent);
...
/*
            if (position == initiationSessions.size() - 1 && !showMoreProgressBar) {
    ...
    private void shareTextUrl(PojoUpcomingSession currentItem) {
    ...
    public int getItemCount() {
        if (listSize != 0) {
            ...
            return initiationSessions.size() + 1;
        }
    ...
    private void openShareSessionOnProfileDialog(final PojoUpcomingSession currentItem) {
    ...
    /*private void openSessionShareDialog(PojoUpcomingSession currentItem) {
        LinearLayout ll_timeline_wrapper, ll_platform_wrapper;
        BottomSheetDialog sessionCreationDialog = new BottomSheetDialog(activity);
        sessionCreationDialog.setContentView(R.layout.share_session_option_dialog);
        ll_timeline_wrapper = (LinearLayout) sessionCreationDialog.findViewById(R.id.ll_timeline_wrapper);
        ll_platform_wrapper = (LinearLayout) sessionCreationDialog.findViewById(R.id.ll_platform_wrapper);
